...
def get_json_raw_data(mdate=None):
    ...
    zero_today = datetime.datetime.strptime(
        mdate, '%Y-%m-%d') if mdate else zero_today
    zero_tomorrow = datetime.datetime(
        zero_today.year, zero_today.month, zero_today.day+1, 0, 0, 0)
    raw_data = get_raw_data()
    json_raw_data = []
    for line in raw_data:
        log_time = line.log_time.replace(tzinfo=None)
        #print(log_time, zero_today)
...
def stat_daily_page_view(mdate=None):
    ...
    this_month = datetime.datetime.strptime(
        mdate, '%Y-%m').month if mdate else today.month
    raw_data = get_raw_data()
